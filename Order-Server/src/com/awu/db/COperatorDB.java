...
public class COperatorDB extends CCommonDB {
	...
	public CDataTable getOperatorList(int page, int pageSize) {
		...
		String sql = "select operator.idoperator,operator.roleid,operator.fullname,"
				+ "operator.username,waiter.age,sex.fullname as sex,waiter.phonenumber, "
				+ "role.fullname as rolename "
				+ "from operator ";
		sql += "left join waiter  on operator.waiterid = waiter.idwaiter ";
		sql += "left join sex on waiter.sex = sex.idsex ";
		sql += "left join role on operator.roleid = role.idrole ";
		...
		try {
			...
			if (!flag)
				System.err
						.println("operator total size less then select size.");
			return dt;
		} catch (Exception e) {
	 ...
	 */
	public EDBMSG addOperator(COperator instance) throws SQLException,
	...
	public String getRoleList() {
		String sql = "select idrole,fullname from role";
		try {
			CDataTable dt = dbUtils.executeQuery(sql);
			return dt.toJson("limit").replace(",\"totalRecord\":0}", "}");
		} catch (Exception e) {
			return "";
		}
	}
